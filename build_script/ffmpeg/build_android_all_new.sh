...
if [[ -z "$1" ]]; then
    echo "Invalid argument! Usage: $0 <FFMPEG_SOURCE_DIR>"
    exit 1
fi
...
for arch in "${ARCHITECTURES[@]}"; do
    ...
    if [[ "$arch" == "armeabi-v7a" ]]; then
        bash "$THIS_DIR/build_android_armeabi_v7a_new.sh" "$FFMPEG_SOURCE_DIR" "$BUILD_ROOT"
    elif [[ "$arch" == "arm64-v8a" ]]; then
        bash "$THIS_DIR/build_android_arm64_v8a_new.sh" "$FFMPEG_SOURCE_DIR" "$BUILD_ROOT"
    elif [[ "$arch" == "x86_64" ]]; then
        bash "$THIS_DIR/build_android_x86_64_new.sh" "$FFMPEG_SOURCE_DIR" "$BUILD_ROOT"
    fi
